        ...
        def wrapper(self):
            for i in self.MATERIALS:
                if i not in self.blacklist:
                    ...
                    x,y = None,0      # patch for "UnboundLocalError: local variable 'x' referenced before assignment"
                    try:
                        x,y = func(i)
                    except:
                        pass
                    # 'above strategy results in brownout because first function will overload tasks if does not exist'
                    # x,y = func(i)
                    if x is not None:
